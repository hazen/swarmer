defmodule Swarmer.Worker do
  ...
  @impl true
  def init([name]) do
    {:ok, {name, :rand.uniform(5_000) + 5_000}, 0}
  end
  ...
  def handle_call({:swarm, :begin_handoff}, _from, {name, delay}) do
    IO.puts("Handing off #{name} with delay #{delay}")
  ...
  def handle_cast({:swarm, :end_handoff, delay}, {name, _}) do
    IO.puts("Ending handoff for #{name} with delay #{delay}")
    {:noreply, {name, delay}}
  end
  ...
  def handle_cast({:echo, msg}, {name, _} = state) do
    IO.puts("#{inspect(name)} says #{msg}")
    {:noreply, state}
  end
  ...
  def handle_info(:soap, {name, delay}) do
    IO.puts("#{inspect(name)} says FIGHT!")
    {:noreply, {name, delay}}
  end
